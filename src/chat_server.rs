...
use rustafarian_shared::logger::LogLevel::{DEBUG, ERROR, INFO};
...
impl ChatServer {
    ...
    fn handle_message_fragment(&mut self, packet: Packet, fragment: Fragment) {
        ...
        self.logger.log(
            format!(
                ...
                sender_id, session_id, fragment_id
            )
            .as_str(),
            INFO,
        );
        ...
        if let Some(message) = self.assembler.add_fragment(fragment, session_id) {
            ...
            self.logger.log(
                format!("A complete message was formed: [{}]", message).as_str(),
                DEBUG,
            );
    ...
    fn handle_ack(&mut self, packet: &Packet, ack: &Ack) {
        ...
        self.logger.log(
            format!("Received ACK from node [{}]", sender_id).as_str(),
            INFO,
        );
        ...
        if let Some(fragments) = self.fragment_sent.get_mut(&session_id) {
            ...
            // then remove it
            fragments.retain(|packet| match &packet.pack_type {
                PacketType::MsgFragment(fragment) => fragment.fragment_index != fragment_id,
                _ => true,
            });
            ...
            if fragments.is_empty() {
                self.fragment_sent.remove(&session_id);
            }
        } else {
            // NO SESSION FOUND
            self.logger.log(
    ...
    fn handle_nack(&mut self, packet: &Packet, nack: &Nack) {
        ...
        self.logger.log(
            format!(
                "Received NACK from node [{}], nack type [{:?}]",
                sender_id, nack_type
            )
            .as_str(),
            INFO,
            ...
            NackType::Dropped => {
                ...
                if let Some(fragments) = self.fragment_sent.get_mut(&session_id) {
                    if let Some(fragment) =
                        fragments.iter().find(|&packet| match &packet.pack_type {
                            PacketType::MsgFragment(fragment) => {
                        ...
                        })
                    {
                        ...
                        let source_header = self
                            .topology
                            .get_routing_header(self.id, destination_id.unwrap());
                        ...
                        let new_packet =
                            Packet::new_fragment(source_header, session_id, new_fragment);
    ...
    fn handle_flood_request(&mut self, packet: &Packet, mut flood_request: FloodRequest) {
        ...
        self.logger.log(
            format!("<- Flood request received from node [{}]", sender_id).as_str(),
            INFO,
        );
        ...
        for (id, sender) in &self.node_senders {
            if *id != sender_id {
                match sender.send(new_flood_request.clone()) {
                    ...
                    Err(e) => {
                        self.logger.log(
                            format!("An error occurred while sending a flood request - [{}]", e)
                                .as_str(),
                            ERROR,
                        );
                    }
    ...
    fn handle_flood_response(&mut self, mut packet: Packet, flood_response: &FloodResponse) {
        ...
        if initiator != self.id {
            ...
            self.logger
                .log("<- New flood response received, updating topology", INFO);
        }
        ...
        for (i, node) in flood_response.path_trace.iter().enumerate() {
            ...
            if !self.topology.nodes().contains(&node.0) {
                self.logger.log(
                    format!("New node [{}] added to the list", node.0).as_str(),
                    DEBUG,
                );
        ...
        if !self.fragment_retry_queue.is_empty() {
    ...
    fn handle_complete_message(
        ...
        match ChatRequestWrapper::from_string(message) {
            Ok(req_wrapper) => {
                self.logger
                    .log("Successfully deserialized the message", DEBUG);
                match req_wrapper {
    ...
    fn handle_client_list_request(&mut self, node_id: NodeId, session_id: u64) {
        self.logger.log(
            format!("<- Received request from [{}] to get client list", node_id).as_str(),
            INFO,
        );

        let response = ChatResponseWrapper::Chat(ChatResponse::ClientList(
            self.registered_clients.clone().into_iter().collect(),
        ));
    ...
    fn handle_register_request(
        ...
        self.logger.log(
            format!("New client [{}] registered!", node_id).as_str(),
            INFO,
        );
        self.registered_clients.insert(node_id);

        // Send response once client is registered
        let response = ChatResponseWrapper::Chat(ChatResponse::ClientRegistered);
    ...
    fn handle_send_message_request(
        ...
        let msg_response = ChatResponseWrapper::Chat(ChatResponse::MessageFrom {
            from: sender_id,
        ...
        self.logger.log(
            format!(
                "-> Confirming message sent successfully to sender [{}]",
                sender_id
            )
            .as_str(),
            DEBUG,
        );
        let msg_response = ChatResponseWrapper::Chat(ChatResponse::MessageSent);
    ...
    fn handle_add_sender_command(&mut self, node_id: NodeId, sender: Sender<Packet>) {
        self.logger.log(
            format!("Adding node [{}] to neighbours", node_id).as_str(),
            INFO,
        );
        ...
        if self.node_senders.contains_key(&node_id) {
            ...
            self.logger.log(
                format!("Node [{}], already found among neighbours", node_id).as_str(),
                ERROR,
            );
            return;
        }
    ...
    fn handle_topology_command(&mut self) {
        self.logger
            .log("Sending current topology to Simulation Controller", INFO);
        ...
        if header.is_empty() {
            ...
            for fragment in fragments {
                ...
                self.fragment_retry_queue
                    .insert((session_id, fragment.fragment_index));
                ...
                let header = SourceRoutingHeader::new(vec![destination_node], 0);
    ...
    fn send_packet(&mut self, packet: Packet) {
        self.logger.log(
            format!(
                "-> Sending a new packet with session [{}]",
                packet.session_id
            )
            .as_str(),
            INFO,
        ...
        if packet.routing_header.hops.is_empty() {
            self.logger
                .log("Packet has an empty route, abort sending!", ERROR);
            return;
        }
        ...
        if let PacketType::MsgFragment(_) = packet_type.clone() {
            ...
            self.topology
                .update_node_history(&packet.routing_header.hops, false);
        }
        ...
        match self.node_senders.get(&node_id) {
            ...
            None => {
                self.logger.log(
                    format!("No node found with id [{}]", node_id).as_str(),
                    ERROR,
                );
            }
        }
    ...
    fn send_ack(
        ...
        let mut routing_header = self.topology.get_routing_header(self.id, destination_node);

        // If no route was computed, use the fallback one
        if routing_header.is_empty() {
            self.logger
                .log("No route computed, using fallback one", DEBUG);
        ...
        self.logger.log(
            format!(
                "New header created - [Destination: {}, Route: {:?}]",
                destination_node, routing_header.hops
            )
            .as_str(),
            DEBUG,
        );
    ...
    fn start_flooding(&mut self) {
        ...
        if !self.can_flood() {
            self.logger
                .log("Server is already waiting on a flood", INFO);
            return;
        }
    ...
    fn resend_fragments(&mut self) {
        ...
        for (session_id, fragment_id) in self.fragment_retry_queue.clone() {
            self.logger.log(
                format!(
                    "Handling fragment: [session: {} - fragment: {}]",
            ...
            // If there are fragments for the current session
            if let Some(fragments) = self.fragment_sent.get_mut(&session_id) {
                if let Some(packet) = fragments.iter().find(|&packet| match &packet.pack_type {
                    PacketType::MsgFragment(fragment) => fragment.fragment_index == fragment_id,
                    _ => false,
                }) {
                    // Compute new route, this way if a flood response has updated the topology
                    ...
                    // Fetch the fragment from the packet
                    let PacketType::MsgFragment(fragment) = packet.clone().pack_type else {
                        // Should never reach this code
                        self.logger.log(
                            "Expecting MsgFragment, received wrong type in resend_fragments",
                            ERROR,
                    ...
                    // Create a new packet and send it
                    let new_packet = Packet::new_fragment(source_header, session_id, fragment);
